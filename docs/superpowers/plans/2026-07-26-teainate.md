...
- Never record a hold whose `caffeinate` process failed to spawn.
...
struct Off: ParsableCommand {
    ...
    @Flag(name: .long, help: "Release every hold.")
    var all = false

    func validate() throws {
        if id == nil && !all {
            throw ValidationError("Specify --id <id> or --all.")
        }
    }

    func run() throws {
        let released = try TeainateService.standard().off(id: id)
        if released.isEmpty {
...
private func status(holds: [HoldStatus] = [], foreign: [ForeignAssertion] = []) -> Status {
    Status(awake: !holds.isEmpty, holds: holds, foreignAssertions: foreign)
}
