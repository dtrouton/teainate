...
- `ucomm` may contain spaces for some processes (e.g. `Claude Helper`); the whitespace-splitting parser splits such names at the first space. Harmless here — the only names matched on are `caffeinate` and `claude`, neither of which contains a space.
