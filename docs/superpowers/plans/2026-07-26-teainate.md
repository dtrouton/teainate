...
If the command reports no Claude Code session in the process tree, fall back to a timer:
...
Returns `awake`, the list of `holds` teainate owns, and `foreign_assertions` — other
processes holding the Mac awake. When the user asks "why won't my Mac sleep?", check
`foreign_assertions`: the answer is often another app, not teainate.
